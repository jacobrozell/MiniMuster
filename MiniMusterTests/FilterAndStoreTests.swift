...
fileprivate typealias Unit = MiniMuster.Unit
...
struct SquadStoreTests {
    func makeUnit() -> (TestDatabase, MiniMuster.Unit) {
        let db = TestDatabase()
        let ctx = db.context
        let army = Army(name: "A", game: "40k", faction: "Space Marines"); ctx.insert(army)
        let u = MiniMuster.Unit(name: "Intercessors (5)", qty: 1, state: "Primed", order: 0)
    ...
    func enableGuards() {
        let (db, u) = makeUnit()
        let ctx = db.context
        let solo = MiniMuster.Unit(name: "Captain", qty: 1, state: "Primed", order: 1)
...
struct UndoServiceTests {
    ...
    private func newArmyDatabase() -> TestDatabase {
        ...
        for (i, state) in ["Primed", "Assembled"].enumerated() {
            let u = MiniMuster.Unit(name: "U\(i)", state: state, order: i); u.army = army; ctx.insert(u)
        }
        try? ctx.save()
        return db
    }
